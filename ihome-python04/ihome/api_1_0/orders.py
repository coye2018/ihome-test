...
def to_dict(value):
    ...
    order_dict = {
        ...
        "img_url": constants.QINIU_URL_DOMAIN + value.house.index_image_url if value.house.index_image_url else "",
        "start_date": value.begin_date.strftime("%Y-%m-%d"),
        "end_date": value.end_date.strftime("%Y-%m-%d"),
        # "ctime": value.create_time.strftime("%Y-%m-%d %H:%M:%S"),
...
def get_user_orders():
    ...
    #用户的身份，用户想要查询作为方可预定别人房子的订单，还是想要作为房东查询别人预定自己房子的订单
    ...
    if orders:
        for order in orders:
            order_dict_list.append(to_dict(order))
